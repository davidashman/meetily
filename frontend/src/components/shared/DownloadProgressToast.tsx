...
function DownloadToastContent({
  ...
  return (
    <div className="flex items-center gap-3 w-full max-w-sm bg-card rounded-lg shadow-lg border border-border p-3 relative">
      ...
      {/* Icon */}
      <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${isComplete ? 'bg-green-500/20' : hasError ? 'bg-red-500/20' : isCancelled ? 'bg-muted' : 'bg-muted'
        }`}>
        {isComplete ? (
      ...
      <div className="flex-1 min-w-0">
        ...
        {hasError ? (
          ...
          <>
            ...
            {/* Progress text */}
            <div className="flex items-center justify-between text-xs text-muted-foreground00">
              <span>
